...
VALUE zj_video_video(int argc, VALUE* argv, VALUE self) {
  ...
  VALUE cached_video = rb_hash_aref(_zj_video_hash, filename);
  ...
  if(NIL_P(cached_video)) {
    ...
    rb_hash_aset(_zj_video_hash, filename, cached_video);
  }
...
void Init_Videos() {
  ...
  rb_define_variable("_zj_image_hash", &_zj_video_hash);
  _zj_video_hash = rb_hash_new();
